...
//
//   GET  /api/metrics?diag=meta                          → config + last 10 orders
//   POST /api/metrics?diag=meta&test_event_code=TEST1234  → live token/payload check
//
// Lives in lib/ (not api/) on purpose: Vercel's Hobby plan caps the project at
// 12 serverless functions, and api/ was already full. Only files in api/ count.
//
// Never returns a secret — only whether each one is present.
import crypto from 'node:crypto';
import { sbSelect, supabaseReady } from './db.js';

// Caller (api/metrics.js) has already checked the admin password.
export default async function metaDiagnose(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const payload = {
      data: [{
        ...
        event_source_url: `${process.env.SITE_URL || 'https://heartnote.music'}/success`,
        user_data: {
