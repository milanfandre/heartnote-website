...
// Any server-side event. `eventId` must match the browser Pixel's eventID for
// the same action, otherwise Meta counts it twice.
export async function sendMetaEvent({ eventName, eventId, email, fbp, fbc, ip, userAgent, value, currency, eventSourceUrl, customData }) {
  if (!metaReady() || !eventName) return;
  ...
  if (value != null) {
    custom_data.currency = (currency || 'usd').toUpperCase();
    custom_data.value = value / 100;
  }
  ...
  try {
    ...
    if (!r.ok) console.error('Meta CAPI', eventName, r.status, await r.text());
  } catch (err) {
