...
const PURCHASE_RACE_MINUTES = 60; // a purchase this recent means they bought, not abandoned
...
async function scheduleSequence(row, from = 0) {
  const steps = SCHEDULE.slice(from);
  // Handed to Resend together rather than one at a time with a pause between.
  //
  // These four used to run in sequence with a 600ms gap, which put the whole
  // capture close to the platform's 10 second ceiling. When it went over, the
  // function was killed part-way through the loop: emails were already live
  // in Resend, but their ids had never been written back, so a later purchase
  // had nothing to cancel. On 2026-09-14 that is how a paying customer was
  // sent "your song is saved" after they had paid.
  //
  // The gap was guarding against Resend's 2-per-second limit. sendEmail
  // already retries a 429 with backoff, which is the better guard, so losing
  // the gap costs nothing and takes the whole call to about one round trip.
  const settled = await Promise.allSettled(steps.map((step) => sendEmail({
    to: row.email,
  ...
  settled.forEach((outcome, i) => {
    if (outcome.status === 'fulfilled' && outcome.value && outcome.value.id) {
      ids.push(outcome.value.id);
      return;
    }
    // One failed schedule must not lose the others.
    const msg = outcome.status === 'rejected' ? outcome.reason.message : 'no id returned';
    console.warn(`abandoned: could not schedule "${steps[i].key}":`, msg);
    errors.push(`${steps[i].key}: ${msg}`);
  });
...
export async function captureAbandoned(payload = {}, attr = null) {
  ...
  // Did they already pay? The webhook cancels the sequence when someone buys,
  // but it can only cancel what exists at the moment it runs. If the purchase
  // completes while this beacon is still in flight, the webhook finds nothing,
  // and then this creates a fresh sequence for a customer who has paid.
  //
  // That is not hypothetical: on 2026-09-14 an order was written at 02:00:15
  // and this capture landed at 02:00:20, and the customer was emailed "your
  // song is saved" and "hear how it could sound" after paying.
  //
  // Bounded to an hour so a genuine second song, ordered later, still gets a
  // sequence of its own.
  // Fails open. sbSelect throws on a bad response, and losing a capture to a
  // momentary database hiccup costs real recovery revenue, while the window
  // this guards is only a few seconds wide and the webhook still covers every
  // purchase that lands after capture.
  try {
    const orderWindow = new Date(Date.now() - PURCHASE_RACE_MINUTES * 60000).toISOString();
    const bought = await sbSelect('orders',
      `select=id&customer_email=ilike.${encodeURIComponent(email)}&created_at=gte.${orderWindow}&limit=1`);
    if (bought.length) return { skipped: 'already purchased' };
  } catch (err) {
    console.warn('abandoned: could not check for a recent order:', err.message);
  }
...
function briefFields(p = {}) {
  return {
    ...
    // The quiz no longer asks for themes or a favorite thing. Those columns
    ...
    // The quiz no longer asks for a tempo, nor for a named singer alongside the
    // male/female choice. Both columns stay for older rows; new ones leave
    // tempo null rather than repurposing it.
    tempo: null,
...
//
// Returns the ids that could NOT be canceled. Those are written back to the
...
async function stopSequence(row) {
  ...
  if (stuck.length) {
    console.error(`abandoned: ${stuck.length} follow-up(s) for ${row.email} could NOT be canceled and will still send`);
  }
  return stuck;
}
