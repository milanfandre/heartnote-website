...
// Sent to the customer when their finished song is delivered.
// `extras` lists any additional formats included (e.g. ['wav', 'zip']).
export function deliveryEmailHTML({ displayName, songTitle, giftUrl, extras = [] }) {
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#2B2019;background:#F7F1E6;padding:32px 24px;border-radius:16px">
    ...
    <p style="color:#6B5D50;line-height:1.6;margin:0 0 ${extraLine ? '16px' : '24px'}">Your custom song, <strong>${esc(songTitle)}</strong>, is finished. Open your private keepsake page to play it, download it, and keep it forever.</p>
    ${extraLine}
