...
// The posting address Stripe collected, laid out ready to copy onto a label.
const shipTo = (s) => {
  const a = s && s.address;
  if (!a) return '';
  const lines = [s.name, a.line1, a.line2, [a.city, a.state].filter(Boolean).join(', '), a.postal_code, a.country]
    .filter((v) => v && String(v).trim());
  return row('Ship to', lines.join('\n'));
};
...
export function orderNotificationHTML(order) {
  ...
  // A 6-hour order has to be impossible to miss in the inbox: banner first,
  // then the row, so it survives both a skim and a search.
  const rush = order.expedited === 'yes'
    ? `<div style="background:#6E1423;color:#F7F1E6;border-radius:10px;padding:12px 16px;margin:0 0 16px;font-size:15px;line-height:1.5"><strong>EXPEDITED ORDER.</strong> Paid for 6-hour delivery. Due by ${esc(order.expedited_due_label || '6 pm Central today')}.</div>`
    : '';
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:640px;margin:0 auto;color:#2B2019">
    ...
    <table style="border-collapse:collapse;width:100%">
      ...
      ${row('Expedited', order.expedited === 'yes' ? 'YES, due by ' + (order.expedited_due_label || '6 pm Central today') : '')}
...
export function lyricsForApprovalHTML({ displayName, lyrics, reviewUrl, revision = 0 }) {
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:640px;margin:0 auto;color:#2B2019">
    ...
    <p style="color:#6B5D50;line-height:1.6;margin:0 0 20px">
      ${revision
        ? `We've made the changes you asked for. Read through ${esc(displayName)}'s song below.`
...
export function orderConfirmationHTML({ recipient, tier, receipt = [], total, currency = 'usd', expedited = false, dueLabel = '' }) {
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#2B2019;background:#F7F1E6;padding:32px 24px;border-radius:16px">
    ...
    ${receiptBlock}
    <p style="color:#6B5D50;line-height:1.6;margin:0 0 16px">${expedited
      ? `You chose expedited delivery, so it will arrive by email at this address by ${esc(dueLabel || '6 pm Central today')}.`
      : `You'll receive it by email at this address, usually within about 24 hours.`} It arrives as a private page where you can play your song, download it, and share it with the people you love.</p>
...
export function deliveryEmailHTML({ displayName, songTitles = [], songTitle, songCount, giftUrl, extras = [], chooseVersions = 0 }) {
  ...
  // Deluxe/Experience: they pick a favorite version on the gift page.
  const intro = chooseVersions
    ? `<p style="color:#6B5D50;line-height:1.6;margin:0 0 20px">Your song is ready in <strong>${chooseVersions} versions</strong>. Listen to each one in full, then choose the one you'd like to keep. It's yours to download and share forever.</p>`
    : count > 1
