...
// Where the order came from: paid ad, organic, or direct. Rendered at the top
// of the internal notification so the team can see which ad earned each sale.
//
// NOTE ON "WHICH AD": a Facebook click only ever gives us `fbclid`, which is
// opaque — it identifies the click to Meta, not the ad to us. The ad/campaign
// names below come from URL tags set in Ads Manager (utm_campaign={{campaign.name}},
// utm_content={{ad.name}}, and so on). Without those tags we can prove it was a
// Meta ad but not which one, so this block says exactly that.
function attributionBlock(attr) {
  if (!attr || typeof attr !== 'object' || !Object.keys(attr).length) {
    return `<div style="background:#FBF7EE;border:1px solid #eadfce;border-radius:8px;padding:10px 14px;margin:0 0 16px">
      <span style="color:#6B5D50">Traffic source: <strong style="color:#2B2019">not recorded</strong> (order placed before source tracking, or the visitor blocked it)</span></div>`;
  }

  const src = String(attr.utm_source || attr.source || '').toLowerCase();
  const isMeta = Boolean(attr.fbclid) || /facebook|instagram|^fb$|^ig$|meta/.test(src)
    || /facebook|instagram/.test(String(attr.site_source_name || '').toLowerCase());

  // The headline verdict, colour-coded so it's readable at a glance.
  let verdict, tint, ink;
  if (isMeta) { verdict = 'Meta ad (Facebook / Instagram)'; tint = '#EAF4EF'; ink = '#1B6B4A'; }
  else if (attr.gclid || src === 'google') { verdict = 'Google ad'; tint = '#EAF4EF'; ink = '#1B6B4A'; }
  else if (src && src !== 'direct') { verdict = `Referral · ${attr.utm_source || attr.source}`; tint = '#FBF3E4'; ink = '#8A6414'; }
  else { verdict = 'Direct or organic (not from a paid ad)'; tint = '#F4F1EC'; ink = '#6B5D50'; }

  // Named ad detail, only when Ads Manager URL tags were actually set.
  const named = attr.utm_campaign || attr.utm_content || attr.utm_term || attr.ad_id;
  const detail = `<table style="border-collapse:collapse;width:100%;margin-top:8px">
      ${row('Campaign', attr.utm_campaign)}
      ${row('Ad', attr.utm_content)}
      ${row('Ad set', attr.utm_term)}
      ${row('Ad ID', attr.ad_id)}
      ${row('Placement', attr.placement)}
      ${row('Medium', attr.utm_medium)}
      ${row('Landing page', attr.landing)}
      ${row('First seen', attr.landed_at ? String(attr.landed_at).replace('T', ' ').slice(0, 16) + ' UTC' : '')}
    </table>`;

  const missing = (isMeta && !named)
    ? `<p style="margin:8px 0 0;color:#8A6414;font-size:13px">Which ad is unknown — Facebook only passes an opaque click id. Add URL tags in Ads Manager to name the campaign and ad here.</p>`
    : '';

  return `<div style="background:${tint};border:1px solid #e4d9c6;border-radius:8px;padding:12px 14px;margin:0 0 16px">
      <div style="font-weight:bold;color:${ink}">Came from: ${esc(verdict)}</div>
      ${detail}${missing}
    </div>`;
}
...
export function orderNotificationHTML(order) {
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:640px;margin:0 auto;color:#2B2019">
    ...
    ${attributionBlock(order.attribution)}
...
export function deliveryEmailHTML({ displayName, songTitles = [], songTitle, giftUrl, extras = [], chooseVersions = 0 }) {
  ...
  const intro = chooseVersions
    ? `<p style="color:#6B5D50;line-height:1.6;margin:0 0 20px">We wrote <strong>${chooseVersions} versions</strong> of your song. Have a listen to each one, then choose the version you'd like to keep. It's yours to download and share forever.</p>`
    : titles.length > 1
