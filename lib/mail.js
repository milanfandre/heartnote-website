...
//
// Never throws: a follow-up that cannot be cancelled must not fail a paid
...
export async function cancelEmail(id) {
  ...
  try {
    ...
    if (r.ok) return true;
    console.warn(`cancelEmail ${id}: Resend ${r.status} ${await r.text()}`);
    return false;
  } catch (err) {
    console.warn(`cancelEmail ${id} failed:`, err.message);
    return false;
  }
}
