...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'send-lyrics') {
      ...
      const base = process.env.SITE_URL || req.headers.origin || `https://${req.headers.host}`;
      const reviewUrl = `${base.replace(/\/+$/, '')}/lyrics/${orderId}`;
      const who = order.recipient_name || order.brief?.recipient_name || 'your loved one';
      let sent = false, sendError = null;
      if (mailReady() && order.customer_email) {
        try {
          await sendEmail({
            to: order.customer_email,
            subject: revision ? 'Your revised lyrics are ready' : `The lyrics for ${who}'s song`,
            html: lyricsForApprovalHTML({ displayName: who, lyrics: text, reviewUrl, revision }),
      ...
      return res.status(200).json({ ok: true, emailed: sent, emailError: sendError, reviewUrl });
    }
