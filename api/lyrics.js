...
async function respond(req, res) {
  ...
  try {
    ...
    if (order.lyrics_status === 'approved') return res.status(409).json({ error: 'These lyrics have already been approved.' });

    const notes = String(feedback || '').trim();
    if (!approved && notes.length < 5) return res.status(400).json({ error: 'Please tell us what you would like changed.' });
    ...
    if (mailReady() && to) {
      try {
        await sendEmail({
          to,
          subject: approved
            ? `Lyrics approved - ${order.recipient_name || order.tier}`
            : `Lyric changes requested - ${order.recipient_name || order.tier}`,
