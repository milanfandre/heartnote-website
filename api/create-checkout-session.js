...
export default async function handler(req, res) {
  ...
  try {
    ...
    // The quiz flow collects the email before checkout and returns there on
    // cancel; the classic form leaves email to Stripe and returns to order.html.
    // 'v2' is the pre-rename name of the same page and is still accepted so a
    // session started on a cached copy does not lose its cancel path.
    const fromQuiz = b.from === 'v3' || b.from === 'v2';
