...
| v2.html | **/v2** (also /v2.html) | **Unlinked concept page** (noindex): the homepage rebuilt in a competitor's journey format, mobile AND desktop layouts (montage hero videos/hero-montage.mp4, swipe deck, cursor-driven order demo videos/demo-checkout.mp4, receipt comparison, 3-column compare chart, sticky mobile CTA). CTAs point at v2-order.html; not in any nav |
| v2-order.html | **/v2-order** | **v2 quiz checkout** (noindex): 7-step quiz (who for, occasion w/ Hot+Trending badges, up-to-5 themes, style+tempo, voice M/F, favorite thing, story w/ progress meter) then email capture (step 2 of 3) then an "Almost there" checkout (letter, reviews, samples, reaction video, package picker). Submits to the SAME create-checkout-session API (quiz extras ride mood/details/other; `email` prefills Stripe; `from:'v2'` returns cancels here). Fires reached_form + add_to_cart like order.html |
| v3.html | **/v3** | **Unlinked A/B control** (noindex): index.html as it stood before the video work (static anniversary hero + "Now playing" keepsake card, no montage, no reactions carousel). Carries the progressive-enhancement fixes only. CTAs go to the classic order.html, so v2-vs-v3 is a clean funnel comparison |
