...
| v1.html | **/v1** | **Version 1, the A/B control** (noindex): index.html as it stood before the video work (static anniversary hero + "Now playing" keepsake card, no montage, no reactions carousel). Carries the rendering-standard fixes only. CTAs go to the classic order.html, so v1-vs-live is a clean funnel comparison |
| v3.html | **/v3** (was /v2) | **Version 3, the competitor-format build** (noindex): the homepage rebuilt in a rival's journey format, mobile AND desktop (montage hero videos/hero-montage.mp4, swipe deck, cursor-driven order demo videos/demo-checkout.mp4, receipt comparison, 3-column compare chart, sticky mobile CTA). CTAs point at /v3-order; not in any nav. This is the page built to run as an ad |
| v3-order.html | **/v3-order** (was /v2-order) | **Version 3 quiz checkout** (noindex): 7-step quiz (who for, occasion w/ Hot+Trending badges, up-to-5 themes, style+tempo, voice M/F, favorite thing, story w/ progress meter) then email capture (step 2 of 3) then an "Almost there" checkout (letter, reviews, samples, reaction video, package picker). Submits to the SAME create-checkout-session API (quiz extras ride mood/details/other; `email` prefills Stripe; `from:'v3'` returns cancels here). Fires reached_form + add_to_cart like order.html |
